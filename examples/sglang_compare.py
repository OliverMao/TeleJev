...
def main() -> None:
    ...
    print("\nJev (SGLang prefill-only readout):")
    print(f"  requests      : {direct_timing['requests']}")
    print(f"  prefill       : {direct_timing['prefill_seconds']:.4f} s  (first request)")
    print(f"  subsequent    : {direct_timing['suffix_seconds']:.4f} s  (radix-cache hits)")
    print(f"  total         : {direct_timing['total_seconds']:.4f} s")
